//! The always-on agent server: wiring for memory, backend, integrations,
//! the agent, Signal, cron, health, and the WebSocket front — shared by
//! the standalone `temple-server` binary and the embedded per-user daemon
//! (`temple --daemon`, which runs the same entry with local tool
//! execution enabled).
...
/// Run the full agent server until SIGTERM/SIGINT: opens the database,
/// starts the agent, connects Signal (when enabled), runs cron, serves
/// the WebSocket API, and drains sessions on shutdown.
///
/// `local_execution` makes filesystem tools run in-process instead of
/// being dispatched to a remote client — the embedded-daemon mode.
